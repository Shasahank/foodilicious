...
const Signup = () => {
  ...
  const onSubmit = (data) => {
    const { email, password, name } = data; // Get name from form data

    createUser(email, password)
      .then((result) => {
        const user = result.user;

        // Pass the correct name to updateUserProfile
        return updateUserProfile(name);
      })
      .then(() => {
        // Access updated user info from auth.currentUser
        const userInfo = {
          name: auth.currentUser.displayName, // Get displayName from auth.currentUser
          email: auth.currentUser.email, // Email is always accessible
        };
